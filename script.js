...
for (let grade = 0; grade < grades.length; grades++) {
  text += grades[grade] ;
} 

function search() {
  // طبّق الخطوة 7و8 أسفل هذا الكومنت
  let randomnumber = Math.floor(Math.random()*grades.length);
  // طبّق الخطوة 9 أسفل هذا الكومنت
 random_grade_div.innerHTML = `<div>${grades[randomnumber]}</div>`;
 random_grade_div.style.backgroundColor = "white"
}
